...
pols_month %>% #GOOD CODE
    ...
    mutate(
        ...
        prez_gop = if_else(prez_gop == 1, "gop", "NA"), #GOOD CODE
        prez_dem = if_else(prez_dem == 1, "dem", "NA"), #GOOD CODE
        pivot_longer(prez_gop, prez_dem,
                     names_to = "president",
                     values_to = "prez_gop", "prez_dem"), #USE PIVOT
        select(-prez_gop, -prez_dem, -day)) #PROBABLY GOOD CODE
...
snp %>% #GOOD CODE
    ...
    mutate(month = month.name[month]) %>% #GOOD CODE
